...
\begin{itemize}
  \item \textbf{RQ1}. What impact does usage examples have on REST API client developer productivity? In this study, we define API client developer productivity by the following metrics: a) number of API tasks performed successfully within a limited amount of time, b) number of trial API calls before a successful API call is found, and c) time spent to successfully complete an API task.
